...
export default function PaymentMethods() {
  ...
  // Show success toast when Stripe redirects back with ?saved=1
  useEffect(() => {
      ...
      {methods.length > 0 ? (
        ...
        <div className="text-center py-10 text-gray-500 border border-dashed border-[rgba(10,55,90,0.16)] rounded-lg bg-white">
          ...
          <p className="text-xs text-gray-400 mt-1">
            {lang === 'es'
              ? 'Agrega una tarjeta para comprar más rápido o guarda una automáticamente al completar tu primera compra.'
              : 'Add a card for faster checkout, or one will be saved automatically after your first purchase.'}
          </p>
        </div>
      )}
    </div>
