...
export default function VenueMapBuilder({ eventId, initialSections, onSaved, onChange, event, isAdmin, seatBuyers }: VenueMapBuilderProps) {
  ...
  const handleDeleteTemplate = async (e: React.MouseEvent, templateId: string, templateName: string) => {
    ...
    try {
      ...
      toast.success(lang === 'es' ? 'Plantilla eliminada con éxito' : 'Template deleted successfully');
      loadDbTemplates();
    } catch (err: any) {
      toast.error(lang === 'es' ? 'Error al eliminar plantilla' : 'Error deleting template');
    }
  };
