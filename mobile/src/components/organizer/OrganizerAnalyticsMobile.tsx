...
type Attendee = { status?: string; sectionName?: string };
type Props = {
  sales: any | null;
  attendees: Attendee[];
  sections: any[];
...
export function OrganizerAnalyticsMobile({ sales, attendees, sections, eventTitle }: Props) {
  ...
  const data = useMemo(() => {
    ...
    const totalOrders = Number(sales?.totalOrders || orders.length || 0);
    ...
    const totalEventCapacity = sections.reduce((sum, section) => {
      ...
      if (type === 'standing') return sum + (Number(section.capacity) || 100);
    ...
    const rawByDay = orders.reduce<Record<string, { date: string; orders: number; tickets: number; revenue: number }>>((acc, o) => {
      const key = dayKey(o.paidAt || o.createdAt);
      if (!key) return acc;
      if (!acc[key]) acc[key] = { date: key, orders: 0, tickets: 0, revenue: 0 };
      acc[key].orders += 1;
      acc[key].tickets += Number(o.ticketCount || 0);
    ...
    return { totalRevenue, totalOrders, totalTickets, scannedTickets, pendingTickets, totalEventCapacity, remainingEventCapacity, averageOrder, scanRate, salesByDay, salesBySection };
  ...
  const cards = [
    ...
    { label: es ? 'Tickets vendidos' : 'Tickets sold', value: String(data.totalTickets), note: `$${data.averageOrder.toFixed(2)} ${es ? 'promedio/orden' : 'avg/order'}`, icon: 'ticket-outline' as const },
  ...
  const onExport = () => {
    const rows: (string | number)[][] = [
      ...
      [es ? 'Tickets vendidos' : 'Tickets sold', data.totalTickets],
